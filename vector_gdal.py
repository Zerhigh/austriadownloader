...
def query_cadastral_data(geo_url, geom, output_file):
    ...
    clipped.to_file(f'{output_file}.gpkg', driver='GPKG', layer='NFL')
        ...
        if isinstance(geometry, shapely.MultiPolygon):
            ...
            for i, sub_geometry in enumerate(geometry.geoms):
                query_cadastral_data(geo_url=geo_url, geom=sub_geometry,
                                     output_file=f'output/{entry.BL}_{entry.Operat}_{entry.prevTime}_{i}')
        else:
            query_cadastral_data(geo_url=geo_url, geom=geometry, output_file=f'output/{entry.BL}_{entry.Operat}_{entry.prevTime}')
