...
images = images.assign(area=images.geometry.area).sort_values(by="area", ascending=False)
for i, rowi in images.iterrows():
    intersecting_geoms = {}

    # get all neighboour geoms which have an intrsection
    for j, rowj in images.iterrows():
    ...
    images.at[i, "geometry"] = rowi.geometry
    ...
    for k, v in intersecting_geoms.items():
        images.at[k, "geometry"] = shapely.difference(v, rowi.geometry)

# remove area attribute as its too large
images.drop(columns=['area'], inplace=True)

cadaster_download_url = 'https://data.bev.gv.at/download/Kataster/gpkg/national'
imagery_download_url = 'https://data.bev.gv.at/download/DOP/'

# Add rows
images['Date'] = images['beginLifeS'].apply(convert_date)
images['prevTime'] = images['Date'].apply(get_previous_timestep)
images['vector_url'] = images.apply(lambda row: f"{cadaster_download_url}/KAT_DKM_GST_epsg31287_{modify_date_acess(row.prevTime)}.gpkg", axis=1)
images['RGB_raster'] = images.apply(lambda row: generate_raster_urls(url_base=imagery_download_url, row=row, channel='RGB'), axis=1)
images['NIR_raster'] = images.apply(lambda row: generate_raster_urls(url_base=imagery_download_url, row=row, channel='NIR'), axis=1)

if TU_PC:
    images = images.set_crs('EPSG:31287')
images.to_file(os.path.join(BASE_PATH, 'intersected_regions', 'ortho_cadastral_matched.shp'))
